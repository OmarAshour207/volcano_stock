...
// If a user is Authenticated then there is no problem user can go for checkout
        ...
        if(Auth::guard('web')->check())
        {
                ...
                $total = floatval(preg_replace("/[^-0-9\.]/","", Session::get('coupon_total')));


                $total = (float)$total + round(0 * $curr->value, 2);
    ...
    public function cashondelivery(Request $request)
    {
        ...
        foreach($cart->items as $prod)
        {
            if($prod['item']['user_id'] != 0)
            {
                ...
                $discount_price = VendorPercentage::where('category_id',$prod['item']->category_id)->where('vendor_id',$prod['item']->user_id)->first()->percentage ??$gs->percentage_commission;
