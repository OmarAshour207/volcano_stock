   ...
   public function addtocart($id)
    {
        ...
        $gs = Generalsetting::findOrFail(1);
            try {
                $percentage = $prod->category()->first()->percentage()->where('vendor_id',$prod->user_id)->first()->percentage ??  $gs->percentage_commission  ;


            } catch (\Exception $e) {

            }
        $prc = $prod->price + $gs->fixed_commission + ($prod->price/100) * $percentage ;
    ...
    public function coupon()
    {
        ...
        if($from <= $today && $to >= $today)
        {
            if($coupon->status == 1)
            {
                ...
                $total_discount = $products->sum();
                $sub_discount = $total - $total_discount;
                ...
                if($coupon->type == 0)
                {
                    ...
                    $val = $total_discount / 100;

                    $sub = $val * $coupon->price;

                    $total = $total_discount - $sub;
                    $data[0] = round($total + $sub_discount,2);
                    if($gs->currency_format == 0){
                        $data[0] = $curr->sign.$data[0];
                    }
                    else{
                    ...
                    Session::put('already', $code);
                    $total = $total_discount - round($coupon->price * $curr->value, 2);
            ...
            if($coupon->status == 1)
            {
               ...
               if (count($products) <= 0) {
                ...
                if($coupon->type == 0)
                {
                    ...
                    $total =   $total_discount -  $_GET['shipping_cost'];

                    $val = $total / 100;
                    $sub = $val * $coupon->price;
                    $total = $total - $sub;
                    $total = $total + $_GET['shipping_cost'] + $sub_discount;
